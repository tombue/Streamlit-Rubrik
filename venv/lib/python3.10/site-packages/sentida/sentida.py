...
class Sentida():
    def __init__(self, lexicon_file="aarup.csv", intensifier_file = "intensifier.csv"):
        """Reads sentiment dictionary (aarup.csv), fixes the encoding, and sets intensifiers."""
    ...
    def fix_unicode(self, df_col):
        """Fixes unicode errors with Æ, Ø, and Å"""
    ...
    def clean_words_upper(self, sentence):
        """Remove punctuation from sentence and turn it into list of words (simple tokenization)"""
        return self.punct_cleaner(sentence).split()

    def clean_words_lower(self, text):
        """Removes punctuation from sentence, making letters lowercase, and turns into list of words (simple tokenization)"""
    ...
    def caps_modifier(self, sentiments, words):
        """Modifing the sentiment score of words that are written in uppercase"""
    ...
    def get_negator_affected(self, words):
        """Identifying negations in a list of words. Returns list of positions affected by negator"""
        positions = []
    ...
    def get_intensifier(self, sentiments, word_list):
        """Get all intensifiers in a word_list"""
    ...
    def men_sentiment(self, sentiments, words):
        """Modifies sentiment of words before or after 'men' (but) in list of words -- need empirically tested weights (currently [0.5, 1.5] (Hutto & Gilbert, 2014) for the part before and after the 'men's'"""
        for i in range(len(sentiments)):
    ...
    def stemming(self, words):
        """Stems words of sentence using Snowball stemmer -- it has severely limited accuracy in Danish"""
    ...
    def sentida(self, text, output = "mean", normal = True, speed = "normal"):
        """
        Sentida. Takes in a Danish string and returns the sentiment value(s) of that string. Has settings to change the output format, increase the speed, and change the sentiment spectrum.

        Keyword arguments:
        test -- the raw text string
        output -- [mean, total, by_sentence_mean, by_sentence_total] defines return format [mean of all text, total of all text, a list of mean for each sentence, a list of total for each sentence]
        normal -- normalize the result? True by default, otherwise from -5 to 5
        speed -- [normal, fast]: Fast provides 180% speed improvement with drastic accuracy decrease
        """

        if output not in ['mean', 'total', 'by_sentence_mean', 'by_sentence_total']:
            raise NameError("Not a valid output type. Use either 'mean', 'total', 'by_sentence_mean', and 'by_sentence_total'. You typed '", output, "'.")

        if speed not in ["normal", "fast"]:
            raise NameError("Not a valid speed setting. Use 'normal' or 'fast'. You wrote '", speed, "'.")
        ...
        if speed  == "fast":
            ...
            elif output == "by_sentence_total": # by_sentence_total
